...
// https://rosettacode.org/wiki/Linar_congruential_genertor
uint rand() {
...
struct proc *hold_lottery(int total_tickets) {
    ...
    uint winner_ticket_number = random_number % (uint)total_tickets;
...
// check ptable for matching pid
int
validproc(int proc_id, int num_tickets)
{
  struct proc *p;
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    if (p->pid == proc_id) {
